# ligue1_update.py
# Lille wrapper for at hente 25/26-sæsonen for Ligue 1 og merge med historik.
import os, sys
import pandas as pd
# Hvis denne fil ligger i en undermappe, aktiver projektrod på importstien:
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

from add_newest_season_functions import LeagueSeasonConfig, run_update
from build_training_set_functions2 import XYConfig, build_xy
...
cfg = LeagueSeasonConfig(
    ...
    elo_pickle="elo_dict_premier_league.pkl",
    raw_relpath="Raw/25_26.xlsx",
    prev_seasons_relpath="Alle_tidligere_seasons_med_elo.xlsx",
    combined_out_relpath="Alle_kampe_med_elo.xlsx",
)

info = run_update(cfg, sleep_range=(0.9, 1.8))
print(
    "OK – hentet og flettet.\n"
    f"Rå fil: {info['raw_xlsx']}\n"
    f"Kombineret fil: {info['combined_xlsx']}\n"
    f"Rækker i ny sæson: {info['n_rows_new']}\n"
    f"Total rækker nu: {info['n_rows_total']}"
)


# Indlæs dine to filer (skift til dine egne filnavne)
df_master = pd.read_excel("English Premier League/Alle_kampe_med_elo.xlsx")   # korrekt rækkefølge
df_shuffle = pd.read_excel("English Premier League/Odds_forkert_rækkefølge.xlsx")  # skal sorteres

assert len(df_master) == len(df_shuffle), "Datasættene har forskelligt antal kampe!"

# --- Normalisering af holdnavne ---
ALIASES = {
...
sorted_odds_path = "English Premier League/odds_kampe_sorteret.xlsx"
df_aligned.to_excel(sorted_odds_path, index=False)
print(f"Færdig! Ny fil gemt som {sorted_odds_path}")

# 2) Genindlæs odds-filen og læs kolonnerne V:AP
odds_V_AO = pd.read_excel(sorted_odds_path, usecols="V:AP")
...
cfg = XYConfig(
    ...
    rolling_window=5,
    include_delta=True,
    use_bookmaker_features=True
)

info = build_xy(cfg)
print("Done.\n"
      f"X (rå -> filtreret): {info['X_shape_raw']} -> {info['X_shape']}\n"
      f"Y: {info['Y_shape']}\n"
      f"Gemt X: {info['saved_X']}\n"
      f"Gemt Y: {info['saved_Y']}")
