...
HomeWinProb = pd.read_excel("Alternative modeller (forsøg)/alle_ligaer_sorteret_efter_dato.xlsx", usecols="AV", skiprows=0, nrows=20000)
DrawProb = pd.read_excel("Alternative modeller (forsøg)/alle_ligaer_sorteret_efter_dato.xlsx", usecols="AW", skiprows=0, nrows=20000)
AwayWinProb = pd.read_excel("Alternative modeller (forsøg)/alle_ligaer_sorteret_efter_dato.xlsx", usecols="AX", skiprows=0, nrows=20000)
...
matches = pd.read_excel("Alternative modeller (forsøg)/alle_ligaer_sorteret_efter_dato.xlsx", usecols=columns_to_use)
matches2 = pd.read_excel("Alternative modeller (forsøg)/alle_ligaer_sorteret_efter_dato.xlsx", nrows=20000)

matchResults = pd.read_excel("Alternative modeller (forsøg)/alle_ligaer_sorteret_efter_dato.xlsx", usecols="B", skiprows=0, nrows=20000)
...
mResults = matchResults.loc[mask_complete].reset_index(drop=True)
predictedGames = matches2.loc[mask_complete].reset_index(drop=True)

print(f"Form før filter: X={X.shape}, y={getattr(y, 'shape', (len(y),))}")
print(f"Form efter filter: X={X_clean.shape}, y={y_aligned.shape}")

# 8) Gem til Excel
X_clean.to_excel("Alternative modeller (forsøg)/X.xlsx", index=False)
y_aligned.to_excel("Alternative modeller (forsøg)/Y.xlsx", index=False)
mResults.to_excel("Alternative modeller (forsøg)/M.xlsx", index=False)
predictedGames.to_excel("Alternative modeller (forsøg)/predicted_matches.xlsx", index=False)
