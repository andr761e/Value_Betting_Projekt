...
cfg = LeagueSeasonConfig(
    ...
    elo_pickle="elo_dict_ligue_1.pkl",
...
df_master  = pd.read_excel("French Ligue 1/Alle_kampe_med_elo.xlsx")                 # korrekt rækkefølge
df_shuffle = pd.read_excel("French Ligue 1/Odds_forkert_rækkefølge.xlsx")  # skal sorteres
...
BLOCK_SIZES = [380, 380, 380, 279, 380, 380, 380, 306, 305, 27]
...
sorted_odds_path = "French Ligue 1/odds_kampe_sorteret.xlsx"
df_aligned.to_excel(sorted_odds_path, index=False)
print(f"Færdig! Ny fil gemt som {sorted_odds_path}")

# 2) Genindlæs odds-filen og læs kolonnerne V:AO
...
# Gem som før
master_out_path = "French Ligue 1/Alle_kampe_med_elo_og_odds.xlsx"
...
cfg = XYConfig(
    ...
    y_nrows=7981,                # sæt evt. 7981 hvis du vil matche gamle runs 1:1
    out_X="X.xlsx",
    out_Y="Y.xlsx",
    rolling_window=5,
    include_delta=True
...
print(
    ...
    f"Y: {info['Y_shape']}\n"
    f"Gemte X: {info['saved_X']}\n"
    f"Gemte Y: {info['saved_Y']}"
)