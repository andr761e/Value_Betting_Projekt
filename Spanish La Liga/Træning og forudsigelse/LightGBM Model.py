...
# Indlæs data
X = pd.read_excel("Spanish La Liga/X.xlsx").to_numpy()
Y = pd.read_excel("Spanish La Liga/Y.xlsx").to_numpy()
...
matches = pd.read_excel("Spanish La Liga/Alle_kampe_med_elo_og_odds.xlsx", usecols=columns_to_use)

# load elo_dict
with open("Spanish La Liga/elo_dict_la_liga.pkl", "rb") as f:
...
fixtures = np.array([
    ["Sevilla", "Elche", 2.15, 3.40, 3.40],
    ["Getafe", "Oviedo", 1.90, 3.10, 4.50],
    ["Real Sociedad", "Real Madrid", 5.25, 3.80, 1.65],
    ["Athletic Club", "Alavés", 1.57, 4.00, 6.00],
    ["Atlético Madrid", "Villarreal", 1.90, 3.70, 3.80],
    ["Celta Vigo", "Girona", 1.65, 3.90, 5.00],
    ["Levante", "Real Betis", 3.30, 3.40, 2.15],
    ["Osasuna", "Rayo Vallecano", 2.35, 3.30, 3.10],
    ["Barcelona", "Valencia", 1.20, 7.50, 11.00],
    ["Espanyol", "Mallorca", 2.15, 3.50, 3.30]
], dtype=object)
...
df_Xpred.to_excel("Spanish La Liga/Træning og forudsigelse/X_pred.xlsx", index=False)